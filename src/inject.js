...
const DL_EL2 = '<button class="tw-button tw-button--dropmenu tw-button--hollow tw-interactive clip-dl-button tw-item-order-3" data-test-selector="button-text" data-a-target="video-type-filter-dropdown" style="margin-top: 10px;"><span class="tw-button__text" data-a-target="tw-button-text">{TITLE}</span><span class="tw-button__icon tw-button__icon--right"><div style="width: 2rem; height: 2rem;"> <div class="tw-align-items-center tw-full-width tw-icon tw-icon--fill tw-inline-flex"><div class="tw-aspect tw-aspect--align-top tw-svg"></div></div></div></span></button>'.reAll('{TITLE}', '다운로드')
const dlBtn = $(DL_EL)
...
function init() {
    ...
    const invl = setInterval(() => {
            const mode = utils.getMode()
            const target = document.querySelectorAll(CL_CARD)
            if (mode == utils.MODE_LIST || mode == utils.MODE_SINGLE_WITH_LIST) {
